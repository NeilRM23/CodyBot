...
def specialStrategyPath(jsonResponse):
    ...
    pure_Dijkstra(copyGraph.dictNodes, [], enemyNode.id)
    
    selectedNodesList = list()
    for id, node in copyGraph.dictNodes.items():
        if node.costToReach == 2 and type(node) is Node:
    ...
    for node in selectedNodesList:
        ...
        for connection in node.pathConnections:
            ...
            if previousDirection is None:
                ...
                #if currentDirection == previousDirection and type(graphObject.dictNodes[connection.fromNode]) is Node:
    ...
    if goalNode.id in listIdGoals:
        confirmation_flag = True
        for node in selectedNodesList:
            #print("AAAAAAAAAA", goalNode.id, node.id)
